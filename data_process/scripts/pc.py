...
def rm_nan(points: np.ndarray):
    # 删除点云中的nan点
    bool_nan = np.isnan(points)
    inx = np.where(bool_nan)
    points_new = np.delete(points, inx[0], axis=0)
    return points_new
...
def make_xyz_point_cloud(xyz):
    ...
    xyz = np.rec.fromarrays([xyz[:, 0], xyz[:, 1], xyz[:, 2]], dtype=dt)
    md = {
        "version": 0.7,
        "fields": ["x", "y", "z"],
        "size": [4, 4, 4],
    ...
    pc = PointCloud(md, xyz)
    return pc


def make_xyz_rgb_point_cloud(xyz_rgb):
    """Make a pointcloud object from xyz array."""
    dt = np.dtype(
        [("x", np.float32), ("y", np.float32), ("z", np.float32), ("rgb", np.uint32)]
    )
    xyz_rgb = np.rec.fromarrays(
        [xyz_rgb[:, 0], xyz_rgb[:, 1], xyz_rgb[:, 2], xyz_rgb[:, 3]], dtype=dt
    )
    md = {
        "version": 0.7,
        "fields": ["x", "y", "z", "rgb"],
...
def make_xyz_intensity_point_cloud(xyzl, label_type="u"):
    ...
    if label_type.lower() == "f":
        ...
        xyzi = np.rec.fromarrays(
            [xyzl[:, 0], xyzl[:, 1], xyzl[:, 2], xyzl[:, 3]], dtype=dt
        )
        field_type = ["F", "F", "F", "F"]
    if label_type.lower() == "u":
        dt = np.dtype(
            [
                ("x", np.float32),
                ("y", np.float32),
    ...
    pc = PointCloud(md, xyzi)
    return pc


def make_xyz_label_point_cloud(xyzi, label_type="u"):
    ...
    if label_type.lower() == "f":
        ...
        xyzl = np.rec.fromarrays(
            [xyzi[:, 0], xyzi[:, 1], xyzi[:, 2], xyzi[:, 3]], dtype=dt
        )
        field_type = ["F", "F", "F", "F"]
...
class PointCloud(object):
    ...
    def points_np(self):
        # 只返回坐标点信息
        return np.vstack((self.pc_data["x"], self.pc_data["y"], self.pc_data["z"])).T

    @property
    def data_np(self):
        # return pcd body as numpy
        return np.array(self.pc_data.tolist())

    @property
    def no_nan_points(self):
        # returns points not have nan
        return rm_nan(self.points_np)
    ...
    @classmethod
    def from_ply(cls, ply_file):
        # 只读取ply里面的点信息，会损失掉面信息
        ply_data = PlyData.read(ply_file)
        pcd = ply_data["vertex"].data
        pcd_data = np.asarray(list(pcd))
        points = np.vstack((pcd_data["x"], pcd_data["y"], pcd_data["z"])).T
        return make_xyz_point_cloud(points)

    @classmethod
    def from_stl(cls, stl_file):
        # 只读取stl里面的点信息，会损失掉面信息
        mesh_stl = o3d.io.read_triangle_mesh(stl_file)
        ...
        return make_xyz_point_cloud(V_mesh)

    @classmethod
    def from_obj(cls, obj_file):
        # 只读取obj里面的点信息，会损失掉面信息
        points = []
        ...
        for line in lines:
            ...
            # elif line_list[0]=='f':
            #     faces.append([int(line_list[1]) - 1, int(line_list[2]) - 1, int(line_list[3]) - 1])
            # else:
            #     continue
        return make_xyz_point_cloud(np.asarray(points))
